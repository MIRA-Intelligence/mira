...
def manager(tmp_path: Path) -> SessionManager:
    with patch("radiologybot.session.manager.get_legacy_sessions_dir", return_value=tmp_path / "legacy"):
...
def test_legacy_session_migrated(tmp_path: Path) -> None:
    ...
    with patch("radiologybot.session.manager.get_legacy_sessions_dir", return_value=legacy_dir):
