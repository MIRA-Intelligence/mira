...
class TestBuildRuntimeContext:
    def test_channel_and_chat_id(self) -> None:
        with (
            patch("radiologybot.agent.context.datetime") as m_dt,
            patch("radiologybot.agent.context.time.strftime", return_value="TZ"),
        ):
    ...
    def test_project_dir(self) -> None:
        with (
            patch("radiologybot.agent.context.datetime") as m_dt,
            patch("radiologybot.agent.context.time.strftime", return_value="UTC"),
        ):
            m_dt.now.return_value.strftime.return_value = "T"
...
@patch("radiologybot.agent.context.ContextBuilder._load_builtin_template")
...
@patch("radiologybot.agent.context.SkillsLoader")
@patch("radiologybot.agent.context.MemoryStore")
