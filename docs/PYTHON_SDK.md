...
Use medpilot programmatically — load config, run the agent, get results.

## Quick Start

```python
import asyncio
from medpilot import MedPilot

async def main():
    bot = MedPilot.from_config()
...
### `MedPilot.from_config(config_path?, *, workspace?)`

Create a `MedPilot` from a config file.

| Param | Type | Default | Description |
|-------|------|---------|-------------|
| `config_path` | `str \| Path \| None` | `None` | Path to `config.json`. Defaults to `~/.medpilot/config.json`. |
...
```python
from medpilot.agent import AgentHook, AgentHookContext
...
async def main():
    bot = MedPilot.from_config(workspace="/my/project")
