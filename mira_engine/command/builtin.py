...
async def cmd_stop(ctx: CommandContext) -> OutboundMessage:
    ...
    loop = ctx.loop
    msg = ctx.msg
    project_ref = loop._project_ref_from_metadata(msg.metadata)
    scoped_key = loop._scoped_session_key(project_ref, msg.session_key)
    cancelled, sub_cancelled, background_cancelled = await loop._cancel_session_work(scoped_key)
...
async def cmd_restart(ctx: CommandContext) -> OutboundMessage:
    ...
    async def _do_restart():
        ...
        os.execv(sys.executable, [sys.executable, "-m", "mira_engine"] + sys.argv[1:])
...
async def cmd_status(ctx: CommandContext) -> OutboundMessage:
    ...
    try:
        from mira_engine.utils.searchusage import fetch_search_usage
