...
from radiologybot.agent.tools.base import Tool
...
def _resolve_path(
    ...
    if allowed_dir:
        # Check against local project workspace
        try:
        ...
        # Check against global workspace
        global_workspace = get_workspace_path(None).resolve()
        try:
            resolved.relative_to(global_workspace)
            return resolved
        except ValueError:
            pass
            
        raise PermissionError(
            f"Path {path} is strictly outside allowed directories (Project Workspace: {allowed_dir} or Global Workspace: {global_workspace})"
        )
    return resolved
