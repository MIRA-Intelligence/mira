...
from medpilot.providers.openai_responses import (
    consume_sdk_stream,
    convert_messages,
    convert_tools,
...
if TYPE_CHECKING:
    from medpilot.providers.registry import ProviderSpec
...
_DEFAULT_OPENROUTER_HEADERS = {
    "HTTP-Referer": "https://github.com/HKUDS/medpilot",
    "X-OpenRouter-Title": "medpilot",
...
def _uses_openrouter_attribution(spec: "ProviderSpec | None", api_base: str | None) -> bool:
    """Apply MedPilot attribution headers to OpenRouter requests by default."""
...
class OpenAICompatProvider(LLMProvider):
    ...
    def _build_kwargs(
        ...
        if prefers_max_completion_tokens or (not spec) or getattr(spec, "supports_max_completion_tokens", False):
    ...
    async def chat_stream(
        ...
        idle_timeout_s = int(os.environ.get("MEDPILOT_STREAM_IDLE_TIMEOUT_S", "90"))
        try:
            if self._should_use_responses_api(model, reasoning_effort):
                try:
                    body = self._build_responses_body(
