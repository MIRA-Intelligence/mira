...
from medpilot.config.loader import get_config_path
...
def get_workspace_path(workspace: str | None = None) -> Path:
    ...
    path = Path(workspace).expanduser() if workspace else Path.home() / ".medpilot" / "workspace"
...
def get_bridge_install_dir() -> Path:
    ...
    return Path.home() / ".medpilot" / "bridge"


def get_legacy_sessions_dir() -> Path:
    """Return the legacy global session directory used for migration fallback."""
    return Path.home() / ".medpilot" / "sessions"
