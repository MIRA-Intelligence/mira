"""Utility functions for medpilot."""
...
def get_medpilot_dir(workspace: Path) -> Path:
    """Return the runtime metadata directory under a workspace."""
    return workspace / ".medpilot"
...
_TOOL_RESULTS_DIR = ".medpilot/tool-results"
_TOOL_RESULT_RETENTION_SECS = 7 * 24 * 60 * 60
_TOOL_RESULT_MAX_BUCKETS = 32
_RUNTIME_BOOTSTRAP = ("AGENTS.md", "SOUL.md", "USER.md", "TOOLS.md", "HEARTBEAT.md")
...
def build_status_content(
    *,
    ...
    lines = [
        f"\U0001f408 medpilot v{version}",
...
def sync_workspace_templates(workspace: Path, silent: bool = False) -> list[str]:
    ...
    for item in tpl.iterdir():
        if item.name in _RUNTIME_BOOTSTRAP:
            continue
